...
/*
// Turns a logo image into a black-and-white mask with a beveled edge.
// White / fully-transparent pixels are treated as "outside" the shape; the
// interior is filled via a Poisson solve so the shader gets a smooth bevel.

export function parseLogoImage(src) {
  const canvas = document.createElement('canvas')
  const ctx = canvas.getContext('2d')

  return new Promise((resolve, reject) => {
    if (!src || !ctx) {
      reject(new Error('Invalid source or canvas context'))
      return
    }
    ...
    img.onload = function () {
      ...
      // Identify boundary pixels (a shape pixel with a non-shape neighbor).
      ...
      // Poisson solve: Δu = -C with u=0 on the boundary -> smooth interior bump.
