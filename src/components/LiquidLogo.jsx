...
// Liquid-metal logo renderer (WebGL2). Expects pre-processed `imageData` whose
// red channel encodes a beveled edge mask (see lib/parseLogoImage.js).
...
export default function LiquidLogo({ imageData, params, className = '', onError }) {
  const canvasRef = useRef(null)
  const [gl, setGl] = useState(null)
  const [uniforms, setUniforms] = useState(null)
  const totalAnimationTime = useRef(0)
  const lastRenderTime = useRef(0)
  // Latest params, read inside the rAF loop without re-subscribing every frame.
  ...
  // Compile + link the shader program once on mount.
  useEffect(() => {
    const canvas = canvasRef.current
    const context = canvas?.getContext('webgl2', { antialias: true, alpha: true })
    if (!canvas || !context) {
      fail('Failed to initialize shader. Does your browser support WebGL2?')
      return
    }
    ...
    if (!program || !vertexShader || !fragmentShader) {
      fail('Failed to create program or shaders')
      return
    }
    ...
    if (!context.getProgramParameter(program, context.LINK_STATUS)) {
      fail('Unable to link shader program: ' + context.getProgramInfoLog(program))
      return
    }
    ...
    for (let i = 0; i < uniformCount; i++) {
      ...
      if (!name) continue
      programUniforms[name] = context.getUniformLocation(program, name)
    }
  ...
  // Push non-time uniforms whenever params change.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function render(currentTime) {
      ...
      totalAnimationTime.current += deltaTime * paramsRef.current.speed
  ...
  // Size the canvas and set ratio uniforms.
  useEffect(() => {
  ...
  // Upload the processed logo as a texture.
  useEffect(() => {
    ...
    try {
      ...
      fail('Failed to upload image texture: ' + e)
    }
