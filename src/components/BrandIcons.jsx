...
function BrandIcon({ svg, title, className, viewBox = VIEWBOX, ...props }) {
  return (
    <svg
      viewBox={viewBox}
      className={className}
...
export function AwsLogo({ className = '', ...props }) {
  // The official "aws" wordmark + smile is wider than tall. Crop the viewBox to
  // its content and size by height (width auto) so it sits at the same optical
  // height as the square Azure/GCP marks instead of shrinking to fit a square.
  return (
    <BrandIcon
      svg={AWS_SVG}
      viewBox="0 24 128 80"
      title="Amazon Web Services"
      className={className.replace(/\bw-\d+\b/g, 'w-auto')}
      {...props}
    />
  )
}
...
// Official Kubernetes mark (blue helm + white wheel). Rendered as an <img> from
// an asset file since it keeps its own fill colors and is too large to inline.
export function KubernetesLogo({ className = '', ...props }) {
  return (
    <img
      src={kubernetesUrl}
      alt="Kubernetes"
